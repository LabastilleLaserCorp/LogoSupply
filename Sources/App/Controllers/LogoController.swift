...
final class LogoController: ResourceRepresentable, EmptyInitializable {
    ...
    func index(_ req: Request) throws -> ResponseRepresentable {
        var logos = [JSON]()
        (1...6).map({logos.append(try! Logo(thumbnailURL: try! req.uri.appendingPathComponent("storage/icon_120x120-\($0).png").makeFoundationURL(), logoURL: nil, publishedAt: Date(), id: "\($0)").makeJSON())})
